...
def chapter1():
    ...
    with st.container(border=True):
            ...
            st.write("""
                    ...
                    さらに、このモデルでは新しく追加されたノードは、古いノードの次数を知っている仮定が必要。\n
                    （新しくコミュニティにやってきた人物が、そのコミュニティの中の交友関係を把握している？）
...
def chapter2():
    ...
    st.write("""
             （見えづらいけど）$p$の値が小さくなればなるほど、次数分布の幅は広くなっている。
             これは「三者閉包」によるものである。\n
             古いノードの近傍を選ぶということは、ネットワークからリンクを一つ選ぶことと他ならない。
             

             
             リンクを選択するとき、そのリンクの端点が与えられた次数ｋを持つ確率は、次数ｋに比例するからである。\n
             古いノードは、まさに優先的選択のように次数に比例した確率でリンクを受け取ることになる！
             """)
    with st.container(border=True):
        st.write("""
                ##### 「あるネットワークの中に次数１のノードAと次数１００のノードBがあるとする。ネットワークからランダムにリンクを１つ選んだとき、どちらのノードが端点に選ばれやすい？」
                 """)
    
    with st.container(border=True):
        st.header("ランダムウォークモデルまとめ",divider=True)
        st.write("""
                - 新しいノードが古いノードの次数を知っているという仮定がないのでより直観的なモデルである。
                - 三者閉包は暗黙のうちに優先的選択を誘発する。
    ...
    st.write("""
             「友達の友達と友達になる」を継続させてみる。
             ノード$j$から１ステップ以上のランダムウォークを行いそのウォークが通過したノードとノード$i$を接続するモデルを考えてみた。
             """)
